...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$other_skills  <- readr::read_csv(paste0(data_location, 
                                               "other_skills.csv"), skip = 1)
    cv$text_blocks   <- readr::read_csv(paste0(data_location, 
                                               "text_blocks.csv"), skip = 1)
    cv$contact_info  <- readr::read_csv(paste0(data_location,
                                               "contact_info.csv"), skip = 1)
  }
...
#' @description Construct a bar chart of language skills
#' @param out_of The relative maximum for skills. Used to set what a
#'  fully filled in skill bar is.
print_lang_skill <- function(cv, out_of = 5, bar_color = "#969696",
...
>{skill}</div>"
  ...
  cv$lang_skills %>% dplyr::arrange(dplyr::desc(level)) %>%
...
#' @description Construct a bar chart of other skills
#' @param out_of The relative maximum for skills. Used to set what a
#'  fully filled in skill bar is.
print_other_skill <- function(cv, out_of = 5, bar_color = "#969696",
                             bar_background = "#d9d9d9",
                             glue_template = "default"){
